...
params:
  ...
  project_name: "OPE TempO-Seq"        # Change depending on your project name
...
if (length(n > 0)) {
  ...
  d[flag_cluster,] %>% dplyr::select(original_names, dendro_color_by)
}
```
...
Samples less than `r align_threshold` aligned reads removed. Currently, this only applies to TempO-Seq experiments.

```{r fraction_mapped_reads}
if (Platform == "TempO-Seq") {
  print("TempO-Seq only... Under development")
  QAQC$FMR <- mu$pct_mapped
...
```{r plate_pos, eval = any(grepl("column", colnames(QAQC_annotated), ignore.case = T))}
...
#  geom_tile(mapping = aes(fill = log(dose+1)), data = QAQC_annotated) +
#  scale_fill_gradient(low = "grey", high = "black") +
  geom_point(mapping = aes(color = FMR_data), data = QAQC_annotated, size = 3) +
    scale_x_discrete(position = "top") +
  coord_fixed() +
  facet_wrap(paste0("~",batch_var)) +
  ggtitle("Number of mapped reads") +
...
```{r plot_chemical_differences, eval=any(grepl("dose", colnames(QAQC_annotated), ignore.case = T))}
