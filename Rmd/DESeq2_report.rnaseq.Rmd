...
params:
  ...
  group_facet: "chemical_time" #!r NA          # If you have many different experimental groups, you may subset the report by specifying a column in the metadata to filter groups, and then setting the group of interest in group_filter
  group_filter:  "Estradiol 24 h" # !r NA  Which group will this report be done on?
  strict_contrasts: F # If TRUE, use BOTH columns (exp, cont) of contrasts file to limit what is included in the report. Use with caution!
  ...
  sampledata_sep: "," # Changed later in this script according to RNASeq vs TempOSeq!
output:
  html_document:
    toc: true
    toc_float: true
...
paths$BMD_output <- paste0(paths$root, "/DEG_output/BMD_and_biomarker_files")
...
if (species == "human") {
  ...
  kegg_organism <- "hsa"
  temposeq_manifest <- "181019_Human_S1500_Surrogate_1.2_Manifest.txt"
  # options:
...
##### NEEDS WORK - Use bitr instead?
  ...
  # try_again_2 <- missing[missing$GENE_SYMBOL %in% try_again_2,]
  # 
...
ODAFResults <- annotate_deseq_table(resList, filter_results = F)
...
```{r 'PCA_DEGs'}
rld_degs_names <- significantResults[[biomart_filter]]
if (length(rld_degs_names) > 0) {
  rld_degs <- rld[row.names(assay(rld)) %in% significantResults[[biomart_filter]],] 
  # d[ d[[THECOLUMN]] == someValue , ]
  ## Perform PCA analysis and make plot
  print(plotPCA(rld_degs, intgroup = params$design, ntop = nrow(rld_degs)) +
          theme(legend.position = "bottom") +
          guides(color = guide_legend(ncol = 3)))
...
degs_matrix <- assay(rld_degs)
if (length(degs_matrix) > 0) {
  ## Obtain the sample euclidean distances
...
```{r run_pathway_analysis_child_document, child = pathway_analysis, eval = params$run_pathway_analysis & length(entrezDEGs) > 0 }
```
